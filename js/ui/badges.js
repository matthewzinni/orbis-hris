function getEmployeeRiskMeta(employee) {
    ...
    if (!riskMeta) return ''; const lines = ['At-Risk Flag']; if (riskMeta.manualReason) {
    ...
    if (!impactMeta) return ''; const lines = ['Impact Player']; if (impactMeta.manualReason) {
...
function updateEmployeeRowBadges() {
    const rows = document.querySelectorAll('#employeeTable tbody tr'); rows.forEach(row => {
        const employeeId = row.getAttribute('data-employee-id'); if (!employeeId) return; const employee = EMPLOYEES.find(e => String(e.dbId) === String(employeeId) || String(e.id) === String(employeeId)); if (!employee) return; let badgeContainer = row.querySelector('.employee-badges'); if (!badgeContainer) {
            badgeContainer = document.createElement('div'); badgeContainer.className = 'employee-badges';
            badgeContainer.style.display = 'flex';
            badgeContainer.style.flexDirection = 'column';
            badgeContainer.style.alignItems = 'flex-start';
            badgeContainer.style.gap = '4px';
            badgeContainer.style.marginTop = '6px'; row.querySelector('td:nth-child(2)')?.appendChild(badgeContainer);
        } const riskMeta = getEmployeeRiskMeta(employee);
        const impactMeta = getEmployeeImpactMeta(employee); const riskHtml = buildRiskBadgeHtml(riskMeta);
        const impactHtml = buildImpactBadgeHtml(impactMeta); badgeContainer.innerHTML = `${impactHtml}${riskHtml}`;
        badgeContainer.style.display = 'flex';
        badgeContainer.style.flexDirection = 'column';
        badgeContainer.style.alignItems = 'flex-start';
        badgeContainer.style.gap = '4px';
