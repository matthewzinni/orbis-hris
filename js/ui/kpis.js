...
function renderKpiEmployeeMetrics() {
    // ... other code ...    const turnoverRiskEmployees = reviewEligibleActive.filter(e => {
...
const employees = Array.isArray(window.ALL_EMPLOYEES)
    ...
    : (Array.isArray(window.EMPLOYEES) ? window.EMPLOYEES : (Array.isArray(EMPLOYEES) ? EMPLOYEES : [])); const getEmployeeStatus = (employee) =>
        String(employee.status || employee.displayStatus || employee.employee_status || '')
            .trim()
            .toUpperCase(); const terminatedEmployees = employees.filter(e => {
                const status = getEmployeeStatus(e);
                return status === 'TERMINATED';
            }); const activeEmployees = employees.filter(e =>
                getEmployeeStatus(e) === 'ACTIVE'
            ); const totalWorkforce = activeEmployees.length + terminatedEmployees.length; const turnoverRate = totalWorkforce
                ? ((terminatedEmployees.length / totalWorkforce) * 100).toFixed(1)
                : '0.0'; if (typeof safeSet === 'function') {
...
if (newHireTurnoverSubtext) {
    ...
    * /    / / ... other code ...    if (typeof refreshTurnoverKpisFromSupabase === 'function') {
    refreshTurnoverKpisFromSupabase();
}
}async function refreshTurnoverKpisFromSupabase() {
    const db = window.supabaseClient || (typeof supabaseClient !== 'undefined' ? supabaseClient : null); if (!db) return; const { data, error } = await db
        .from('employees')
        .select('id, first_name, last_name, status, hire_date, tenure_months'); if (error) {
            console.warn('Could not refresh turnover KPIs from Supabase:', error);
            return;
        } const employees = Array.isArray(data) ? data : [];
    console.log('[Turnover KPI] employees:', employees.length, employees.map(e => ({ id: e.id, name: `${e.first_name || ''} ${e.last_name || ''}`.trim(), status: e.status }))); const getStatus = employee => String(employee.status || '').trim().toUpperCase(); const activeEmployees = employees.filter(employee => getStatus(employee) === 'ACTIVE'); const terminatedEmployees = employees.filter(employee => {
    ...
    if (turnoverSubtext) {
        ...
        if (storedTenure > 0) return storedTenure; if (!employee.hire_date) return 0; const hiredAt = new Date(employee.hire_date);
        if (Number.isNaN(hiredAt.getTime())) return 0; const now = new Date();
...
setTimeout(refreshTurnoverKpisFromSupabase, 1500); function buildKpiHoverDetails() {
    // ... other code ...    const turnoverRiskEmployees = getKpiEmployees().filter(e => {
...
});    // ... other code ...    const terminatedNames = getKpiEmployees()
    ...
    .filter(Boolean)
    .sort(compareKpiText); const turnoverCard = document.getElementById('cardTurnover') || document.getElementById('kTurnover')?.closest('.kpi-card, .card, [class*="kpi"]');
if (turnoverCard) {
...
if (turnoverRiskCard) {
    const turnoverRiskNames = getKpiEmployees()
        .map(e => `${e.first || e.first_name || ''} ${e.last || e.last_name || ''}`.trim() || e.displayName || e.name)
        .filter(Boolean)
        .sort(compareKpiText); turnoverRiskCard.title = turnoverRiskNames.length
    ...
    .map(e => `${e.first || e.first_name || ''} ${e.last || e.last_name || ''}`.trim() || e.displayName || e.name)
    .filter(Boolean)
    .sort(compareKpiText); const newHireTurnoverCard = document.getElementById('cardNewHireTurnover') || document.getElementById('kNewHireTurnover')?.closest('.kpi-card, .card, [class*="kpi"]');
...
}    // ... other code ...    // Ensure this is present exactly once at the end:
