...
return {
    ...
    if (!employee) return; if (safeGet('firstName')) safeGet('firstName').value = cleanEmployeeFormNameValue(employee.first_name || employee.first || '');
    ...
    const payload = getEmployeeFormData(); if (!payload.first_name || !payload.last_name) {
        ...
        if (currentEmployee?.id || currentEmployee?.dbId) {
            ...
            return;
        } showToast('Employee saved.'); if (typeof loadEmployees === 'function') {
            await loadEmployees();
