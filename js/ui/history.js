...
return currentEmployee?.dbId || currentEmployee?.id || employeeId;
    ...
    const target = document.getElementById('historyFeed'); if (!actualEmployeeId || !target) return; target.innerHTML = '<div class="empty">Loading history...</div>'; const safeGetByEmployee = async (serviceName) => {
        try {
            const service = window.OrbisServices?.[serviceName]; if (!service || typeof service.getByEmployee !== 'function') {
    ...
    ]); const [notes, meetings, discipline, incidents, reviews] = sources.map(s => s?.data || []); const timeline = [
    ...
    ]; timeline.sort((a, b) => new Date(b.date) - new Date(a.date)); if (!timeline.length) {
        target.innerHTML = '<div class="empty">No history available.</div>';
        return;
    } target.innerHTML = timeline.map(item => {
        const date = item.date ? new Date(item.date + 'T00:00:00').toLocaleDateString() : '—'; return `
