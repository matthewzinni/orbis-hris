...
function buildCandidateInterviewNotice(candidate: any): string {
  ...
  const time = interviewDateTime.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });
...
async function deleteCandidateRecord(): Promise<void> {
  ...
  const { error } = await supabaseClient.from('candidates').delete().eq('id', currentCandidate.id);
  if (error) {
    console.error(error);
...
function renderKpiEmployeeMetrics(): void {
  ...
  const reviewEligibleActive = active.filter(
    (e) =>
      !String(e.payType || '')
        .toLowerCase()
        .includes('contract')
  ...
  const overdueReviewEmployees = reviewEligibleActive.filter((e) => {
    if (!e.nextReview || !(e.nextReview instanceof Date) || isNaN(e.nextReview.getTime()))
      return false;
  ...
  try {
    ...
    if (!manualRiskRes.error) {
      ...
      Object.entries(latestManualRiskByEmployee).forEach(([employeeId, row]: [string, any]) => {
        ...
        if (String(row.note_type || '') === 'At-Risk Flag') {
          OrbisState.atRiskMap[String(employeeId)].manualReason = String(
            row.note_text || ''
          ).trim();
    ...
    const impactPlayers = Object.values(OrbisState.impactPlayerMap || {}).filter(
      (meta: any) =>
        meta && (meta.highReview === true || (meta.manualReason && meta.manualReason.trim() !== ''))
    ).length;
  ...
  try {
    const [meetingsRes, reviewsRes] = await Promise.all([
      ...
      supabaseClient
        .from('employee_reviews')
        .select(
          'employee_id, attendance_score, performance_score, teamwork_score, attitude_score, reliability_score, overall_result, created_at, review_date'
        )
    ...
    if (error) {
      ...
      bodyTarget.innerHTML =
        '<tr><td colspan="6" class="empty">Could not load review dashboard</td></tr>';
      return;
    }
    ...
    if (!reviewsRes.error) {
      (reviewsRes.data || []).forEach((row) => {
        ...
        if (
          ...
          String(sortDate) > String(latestReviewPerformanceByEmployee[employeeId].sortDate)
        ) {
          const scoreValues = [
            row.attendance_score,
            row.performance_score,
    ...
    const reviewRows = activeEmployees
      .map((employee) => {
        ...
        if (
          employee.nextReview &&
          employee.nextReview instanceof Date &&
          !isNaN(employee.nextReview.getTime())
        ) {
      ...
      .sort((a, b) => {
        const aTime =
          a.employee.nextReview instanceof Date && !isNaN(a.employee.nextReview.getTime())
            ? a.employee.nextReview.getTime()
            : Number.MAX_SAFE_INTEGER;
        const bTime =
          b.employee.nextReview instanceof Date && !isNaN(b.employee.nextReview.getTime())
            ? b.employee.nextReview.getTime()
            : Number.MAX_SAFE_INTEGER;
        return aTime - bTime;
      })
      .slice(0, 12);
    ...
    bodyTarget.innerHTML = reviewRows
      .map(
        ({ employee, lastReview, performance, statusLabel, statusClass }) => `
...
async function loadExecutiveInsight() {
  ...
  try {
    ...
    if (error) {
      console.error(error);
      target.innerHTML =
        '<div class="history-item"><div class="history-body">Could not load executive insight.</div></div>';
      return;
    }
    const latestReviewByEmployee = {};
    (data || []).forEach((row) => {
    ...
    const atRiskEmployees = activeEmployees
      .map((employee) => {
        ...
        if (openIncidentCount > 0) {
          reasons.push(
            `${openIncidentCount} open incident report${openIncidentCount === 1 ? '' : 's'}`
          );
      ...
      .sort(
        (a, b) =>
          ...
          compareText(
            `${a.employee.last} ${a.employee.first}`,
            `${b.employee.last} ${b.employee.first}`
          )
      )
      .slice(0, 6);
    if (!atRiskEmployees.length) {
      target.innerHTML =
        '<div class="empty">No employees are currently flagged for immediate HR attention.</div>';
      return;
    }
    target.innerHTML = atRiskEmployees
      .map(
        (item) => `
...
interface RecentActivityItem {
  category: 'Note' | 'Discipline' | 'Meeting' | 'Review';
  title: string;
  date: string;
  employeeId: string;
  body: string;
...
async function loadRecentActivity(): Promise<void> {
  ...
  try {
    ...
    const notes: RecentActivityItem[] = ((notesRes.data || []) as RecentActivityRow[]).map(
      (row) => ({
        category: 'Note',
    ...
    const discipline: RecentActivityItem[] = (
      (disciplineRes.data || []) as RecentActivityRow[]
    ).map((row) => ({
      category: 'Discipline',
    ...
    const meetings: RecentActivityItem[] = ((meetingsRes.data || []) as RecentActivityRow[]).map(
      (row) => ({
        category: 'Meeting',
    ...
    const reviews: RecentActivityItem[] = ((reviewsRes.data || []) as RecentActivityRow[]).map(
      (row) => ({
        category: 'Review',
    ...
    const combined: RecentActivityItem[] = [...notes, ...discipline, ...meetings, ...reviews]
...
function setManualAtRiskUi(flagged, reason = '') {
  currentManualAtRiskState = { flagged: !!flagged, reason: String(reason || '').trim() };
...
async function markEmployeeAtRisk() {
  ...
  const riskKeys = [
    ...
    .filter(Boolean)
    .map(String);
  (window as any).currentAtRiskRosterMap =
    (window as any).currentAtRiskRosterMap || OrbisState.atRiskMap || {};
...
async function clearImpactPlayerStatus() {
  ...
  const employeeDbId = currentEmployee.dbId || currentEmployee.id;
  const noteText =
    String(safeGet('impactPlayerReasonInput')?.value || '').trim() ||
...
async function saveEmployeeRecord() {
  ...
  if (error) {
    console.error(error);
    showToast(
      isCreatingEmployee || !currentEmployee
...
async function deleteEmployeeRecord() {
  ...
  const confirmed = window.confirm(
    `Delete employee ${currentEmployee.first} ${currentEmployee.last}? This cannot be undone.`
  );
  if (!confirmed) return;
...
async function loadEmergencyContacts(employeeId) {
  ...
  if (
    !currentEmergencyContactId ||
    !rows.some((row) => String(row.id) === String(currentEmergencyContactId))
  ) {
    resetEmergencyContactForm();
  }
  target.innerHTML = `
...
async function saveEmergencyContact(): Promise<void> {
  ...
  if (currentEmergencyContactId) {
    ...
    const result = await supabaseClient.from('emergency_contacts').insert([
      {
    ...
    rows.sort((a, b) => {
      ...
      const aOverdue =
        ...
        (String(a.status || '').toLowerCase() === 'overdue' ||
          (a.due_date && String(a.due_date) < today));
      const bOverdue =
        !bDone &&
        (String(b.status || '').toLowerCase() === 'overdue' ||
          (b.due_date && String(b.due_date) < today));
            ...
            <div style="margin-bottom:16px;">
                ...
                ${(items as any[])
                  .map((row) => {
                    ...
                    const dueToday =
                      !done && String(row.status || '').toLowerCase() === 'due today';
                    const metaText = done
                        ...
                        <div class="history-item" style="margin-bottom:10px; border-left:4px solid ${accentColor}; ${cardBackground}">
                            ...
                            <div style="margin-top:8px;">
                                ${
                                    ...
                                    ? '<span class="muted" style="font-size:12px; font-weight:700; color:#0f8a63;">✓ Completed</span>'
                                    : `
...
async function deleteEmergencyContact() {
  ...
  try {
    ...
    const { error } = await supabaseClient.from('emergency_contacts').delete().eq('id', deletingId);
    if (error) {
      console.error(error);
...
async function loadImpactPlayers() {
  ...
  try {
    ...
    target.innerHTML = impactPlayers
      .map(({ emp, impactMeta }: { emp: any; impactMeta: any }) => {
        const metaParts = [];
        if (emp.dept) metaParts.push(emp.dept);
        if (
          impactMeta.highReview &&
          impactMeta.reviewScore !== null &&
          impactMeta.reviewScore !== undefined
        ) {
...
function buildKpiHoverDetails() {
  ...
  const riskEmployees = activeEmployees
    ...
    .map(
      (e) =>
        `${`${e.first || ''} ${e.last || ''}`.trim()}${e.tenureMonths != null ? ` • ${e.tenureMonths} mo` : ''}`
    )
    .filter(Boolean);
  ...
  const atRiskNames = activeEmployees
    .filter((employee) => {
      const keys = [employee.dbId, employee.id, employee.employee_id, employee.displayId]
        ...
        .map(String);
      return keys.some(
        (key) => OrbisState.atRiskMap?.[key] || window.currentAtRiskRosterMap?.[key]
      );
    })
  ...
  const impactPlayerNames = currentFilteredEmployees
    .filter(
      (e) => OrbisState.impactPlayerMap[String(e.dbId)] || OrbisState.impactPlayerMap[String(e.id)]
    )
    .map((e) => `${e.first || ''} ${e.last || ''}`.trim())
    .filter(Boolean)
    .sort(compareText);
  ...
  const dueReviews = activeEmployees
    .filter(
      (e) =>
        e.nextReview &&
        e.nextReview instanceof Date &&
        !isNaN(e.nextReview.getTime()) &&
        e.nextReview <= now
    )
