...
function ensureCandidateModal() {
    ...
    if (drawer) return drawer; drawer = document.createElement('aside');
    ...
    drawer.style.padding = '0'; drawer.innerHTML = `
    <div class="drawer-header" style="background:#0f3554; color:#fff; padding:22px 28px; display:flex; justify-content:space-between; align-items:flex-start; gap:16px;">
    ...
    </div>    <input type="hidden" id="candidateDetailId" />    <div style="padding:20px 28px;">
      <div class="drawer-tabs" style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:18px;">
      ...
      </div>      <div class="drawer-section candidate-tab-panel" data-candidate-panel="profile" style="background:#fff; border:1px solid #e5edf5; border-radius:18px; padding:20px; box-shadow:0 10px 24px rgba(15,23,42,.06); margin-bottom:18px;">
      ...
      </div>      <div class="drawer-section candidate-tab-panel hidden" data-candidate-panel="notes" style="background:#fff; border:1px solid #e5edf5; border-radius:18px; padding:20px; box-shadow:0 10px 24px rgba(15,23,42,.06); margin-bottom:18px; display:none;">
      ...
      </div>      <div class="drawer-section candidate-tab-panel hidden" data-candidate-panel="interview" style="background:#fff; border:1px solid #e5edf5; border-radius:18px; padding:20px; box-shadow:0 10px 24px rgba(15,23,42,.06); margin-bottom:18px; display:none;">
        ...
        <div class="drawer-section" style="background:#f8fafc; border:1px solid #e5edf5; border-radius:16px; padding:16px; margin-top:16px;">
          ...
          </div>          <label class="candidate-field" style="margin-top:14px;">Strengths
        ...
        </div>        <label class="candidate-field" style="margin-top:14px;">Live Interview Notes
      ...
      </div>      <div class="drawer-section candidate-tab-panel hidden" data-candidate-panel="offer" style="background:#fff; border:1px solid #e5edf5; border-radius:18px; padding:20px; box-shadow:0 10px 24px rgba(15,23,42,.06); margin-bottom:18px; display:none;">
        ...
        </label>        <div id="candidateOfferSummary" style="display:none; margin-top:14px; padding:14px 16px; border-radius:14px; background:#ecfdf5; border:1px solid #bbf7d0; color:#064e3b; font-size:13px; line-height:1.5;"></div>        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; margin-top:14px;">
      ...
      </div>      <div class="drawer-section candidate-tab-panel hidden" data-candidate-panel="documents" style="background:#fff; border:1px solid #e5edf5; border-radius:18px; padding:20px; box-shadow:0 10px 24px rgba(15,23,42,.06); margin-bottom:18px; display:none;">
      ...
      </div>      <div class="drawer-section" style="background:#fff; border:1px solid #e5edf5; border-radius:18px; padding:20px; box-shadow:0 10px 24px rgba(15,23,42,.06);">
  ...
  `; const style = document.createElement('style');
    ...
    `; if (!document.getElementById('candidateDrawerStyles')) {
        ...
        const candidateId = drawer.querySelector('#candidateDetailId')?.value; await moveCandidateToNextStage(candidateId);
        ...
        event.preventDefault();
        event.stopPropagation(); const candidateId = drawer.querySelector('#candidateDetailId')?.value; if (!candidateId) {
        ...
        sendCandidateInterviewInvite();
    }); drawer.querySelector('#candidateSaveOfferBtn')?.addEventListener('click', async event => { event.preventDefault(); event.stopPropagation(); console.log('Save Offer button clicked'); await saveCandidateOffer(); }); drawer.querySelector('#candidateSaveDocumentBtn')?.addEventListener('click', async () => {
        await saveCandidateDocument();
    }); document.body.appendChild(drawer);
    return drawer;
} async function openCandidateDetails(candidateId) {
    if (!candidateId) return; const db = getOrbisSupabaseClient();
    if (!db) {
        console.error('Supabase client not found for candidates module.');
        return;
        ...
        .select('*')
        .eq('id', candidateId)
        .single(); if (error || !candidate) {
            console.error('Error opening candidate:', error);
            if (typeof showToast === 'function') showToast('Candidate could not be opened.', 'error');
            return;
        } const drawer = ensureCandidateModal(); document.getElementById('candidateDetailId').value = candidate.id || '';
    ...
    document.getElementById('candidateDetailPhone').value = candidate.phone || ''; if (document.getElementById('candidateDetailNotes')) document.getElementById('candidateDetailNotes').value = candidate.notes || '';
    ...
    if (document.getElementById('candidateInterviewConcerns')) document.getElementById('candidateInterviewConcerns').value = ''; if (document.getElementById('candidateOfferStatus')) document.getElementById('candidateOfferStatus').value = candidate.offer_status || 'Not Started';
    ...
    if (document.getElementById('candidateOfferNotes')) document.getElementById('candidateOfferNotes').value = candidate.offer_notes || ''; const offerSummary = document.getElementById('candidateOfferSummary');
    if (offerSummary) {
    ...
    if (document.getElementById('candidateDocumentNotes')) document.getElementById('candidateDocumentNotes').value = candidate.document_notes || ''; const candidateName = `${candidate.first_name || ''} ${candidate.last_name || ''}`.trim() || candidate.name || candidate.candidate_name || 'Candidate Details';
    document.getElementById('candidateDrawerName').textContent = candidateName;
    document.getElementById('candidateDrawerSubtext').textContent = `${candidate.stage || 'Applied'} • ${candidate.position || candidate.position_applied_for || candidate.role || 'No position listed'}`; drawer.classList.remove('hidden');
    ...
    if (!drawer) return; drawer.querySelectorAll('[data-candidate-tab]').forEach(button => {
    ...
    if (!candidateId) {
        ...
        return;
    } const activeTab = returnTab || document.querySelector('#candidateDetailsDrawer .tab-btn.active')?.dataset.candidateTab || 'profile'; const { error } = await db
        .from('candidates')
        .update(fields)
    ...
    const concerns = document.getElementById('candidateInterviewConcerns')?.value || ''; const evaluationSummary = [
        ...
        : nextStep === 'Reject Candidate'
            ? 'Rejected'
            : 'Interviewing'; await updateCandidateFields({
    ...
    const fields = {
        ...
        .eq('id', candidateId); if (error) {
            ...
            return;
        } candidateToast(`Offer saved: ${offerStatus}.`, 'success'); await refreshCandidatesFromMainApp();
    await openCandidateDetails(candidateId);
    switchCandidateDrawerTab('offer'); const offerSaveStatus = document.getElementById('candidateOfferSaveStatus');
    if (offerSaveStatus) {
    ...
    const db = getOrbisSupabaseClient();
    if (!db) return; const candidateId = document.getElementById('candidateDetailId')?.value;
    if (!candidateId) return; const updatedCandidate = {
    ...
    const candidateId = document.getElementById('candidateDetailId')?.value;
    if (!candidateId) return; const stageInput = document.getElementById('candidateDetailStage');
    if (stageInput) stageInput.value = newStage; await updateCandidateStage(candidateId, newStage);
    await openCandidateDetails(candidateId);
    ...
    if (!db) {
        candidateToast('Supabase connection not found.', 'error');
        return;
    } let candidateId = passedId; if (!candidateId) { candidateId = document.getElementById('candidateDetailId')?.value; } if (!candidateId) { candidateId = await resolveCurrentCandidateIdFromDrawer(db); }
    if (!candidateId) {
        candidateToast('No candidate is selected.', 'error');
    ...
    const normalizedStage = currentStage.toLowerCase().replace(/\s+/g, '');    // Normalization fix for Interviewing/Interview
    const normalizedMap = {
        interviewing: 'interview'
    }; const safeStage = normalizedMap[normalizedStage] || normalizedStage; console.log('Moving candidate to next stage:', { candidateId, currentStage, normalizedStage }); const stages = ['Applied', 'Screening', 'Interview', 'Offer'];
    const currentIndex = stages.findIndex(stage => stage.toLowerCase().replace(/\s+/g, '') === safeStage); let nextStage; if (currentIndex === -1) {
        nextStage = 'Applied';
    } else if (currentIndex === stages.length - 1) {
        nextStage = 'Hired';
    } else {
        nextStage = stages[currentIndex + 1];
    } if (stageInput) stageInput.value = nextStage; await updateCandidateStage(candidateId, nextStage); await openCandidateDetails(candidateId);
} async function convertCurrentCandidateToEmployee() {
    const db = getOrbisSupabaseClient();
    if (!db) {
        candidateToast('Supabase connection not found.', 'error');
        return;
    } const candidateId = await resolveCurrentCandidateIdFromDrawer(db); if (!candidateId) {
        candidateToast('No candidate is selected to hire.', 'error');
    ...
    const candidateId = document.getElementById('candidateDetailId')?.value;
    if (!candidateId) return; await deleteCandidate(candidateId);
    ...
    const position = document.getElementById('candidateDetailPosition')?.value || 'the position'; if (!email) {
    ...
    let data = []; try {
        ...
        .reduce((max, value) => Math.max(max, value), 2500); return `BTW${maxNumber + 1}`;
    ...
    if (window.__isHiringCandidate === true) {
        ...
        return;
    } window.__isHiringCandidate = true; setTimeout(() => {
        ...
        if (!confirmed) {
            ...
            .select('*')
            .eq('id', candidateId)
            .single(); if (fetchError || !candidate) {
                ...
                return;
            } const nextEmployeeId = await getNextEmployeeId(); const drawerCandidateId = document.getElementById('candidateDetailId')?.value || '';
        const drawerMatchesCandidate = String(drawerCandidateId) === String(candidateId); const drawerFirstName = drawerMatchesCandidate ? (document.getElementById('candidateDetailFirstName')?.value || '').trim() : '';
        ...
        const drawerPhone = drawerMatchesCandidate ? (document.getElementById('candidateDetailPhone')?.value || '').trim() : ''; const candidateFullName = `${drawerFirstName || candidate.first_name || ''} ${drawerLastName || candidate.last_name || ''}`.trim()
            || candidate.name
        ...
        const today = new Date().toISOString().slice(0, 10); const employeeRecord = {
        ...
        }; console.log('Creating employee from candidate:', employeeRecord);        // 🔥 CREATE employee USING THE SAME PATH AS MANUAL EMPLOYEE SAVE
        let newEmployee = null;
        let insertError = null; try {
            ...
            const isDuplicateEmployee =
                ...
                String(insertError.details || '').toLowerCase().includes('already exists'); if (!isDuplicateEmployee) {
        ...
        } let onboardingEmployeeId = nextEmployeeId; if (!newEmployee?.id && window.OrbisServices?.employees?.getAll) {
            try {
            ...
            try {
                ...
                const isDuplicate =
                    ...
                    String(onboardingError?.message || '').toLowerCase().includes('duplicate'); if (!isDuplicate) {
            ...
            .from('candidates')
            .delete()
            .eq('id', candidateId); if (deleteError) {
        ...
        await refreshCandidatesFromMainApp();        // 🔥 Ensure roster updates exactly like manual Save Employee
    ...
    if (!confirmed) return; const { error } = await db
        .from('candidates')
        .delete()
        .eq('id', candidateId); if (error) {
