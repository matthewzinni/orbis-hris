window.currentEmergencyContactId = window.currentEmergencyContactId || null; async function ecFetchByEmployee(employeeId) {
    ...
    if (!target) return; const actualEmployeeId = getResolvedEmergencyEmployeeId(employeeId);
    if (!actualEmployeeId) {
        resetEmergencyContactForm();
        target.innerHTML = '<div class="empty">No employee selected.</div>';
        return;
    }
    const { data, error } = await ecFetchByEmployee(actualEmployeeId); if (error) {
        console.error(error);
    ...
    const notes = safeGet('ecNotes')?.value.trim() || ''; if (!contact_name) {
        ...
        return;
    } let error; if (window.currentEmergencyContactId) {
    ...
    const employeeId = getResolvedEmergencyEmployeeId(); if (!confirm('Are you sure you want to delete this emergency contact?')) {
        return;
    } try {
        const { error } = await ecDelete(currentEmergencyContactId); if (error) {
            console.error(error);
            showToast('Could not delete emergency contact.', 'error');
            return;
        } showToast('Emergency contact deleted.'); resetEmergencyContactForm(); if (employeeId) {
            await loadEmergencyContacts(employeeId);
        }
    } catch (err) {
        console.error(err);
