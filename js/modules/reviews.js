function getResolvedReviewEmployeeId(employeeId = null) {
    ...
    const reviewManagerComments = safeGet('reviewManagerComments')?.value || ''; const ratingToScore = (value) => {
    ...
    const trainingScore = ratingToScore(reviewTraining); const blendedAttitudeInputs = [
        communicationScore,
        judgementScore,
        initiativeScore,
        knowledgeScore,
        trainingScore
    ].filter(v => v !== null); const blendedAttitudeScore = blendedAttitudeInputs.length
        ? Math.round(blendedAttitudeInputs.reduce((sum, v) => sum + v, 0) / blendedAttitudeInputs.length)
        : null; const detailedCategorySummary = [
    ...
    ].filter(v => v !== null); const average = avgScore.length
        ? avgScore.reduce((a, b) => a + b, 0) / avgScore.length
        : null; const autoImpactKey = String(employeeId);
            ...
            if (typeof buildKpiHoverDetails === 'function') {
                ...
                if (!currentImpactPlayerRosterMap?.[autoImpactKey]?.persisted) {
                    const existingAutoImpact = await supabaseClient
                        ...
                        .limit(1); if (!existingAutoImpact.error && !(existingAutoImpact.data || []).length) {
    ...
    cancelReviewEdit();    // Core reloads
    await loadEmployeeReviews(employeeId); if (typeof loadSummaryMetrics === 'function') await loadSummaryMetrics();
    if (typeof loadRiskEmployees === 'function') await loadRiskEmployees();
    if (typeof loadImpactPlayers === 'function') await loadImpactPlayers();    // Apply local override ONCE (after DB loads)
    if (autoImpactMeta) {
        currentImpactPlayerRosterMap = currentImpactPlayerRosterMap || {};
        currentImpactPlayerRosterMap[autoImpactKey] = {
            ...(currentImpactPlayerRosterMap[autoImpactKey] || {}),
    ...
    } if (typeof renderRoster === 'function') renderRoster(); if (window.__justPromotedImpactPlayerId && window.__justPromotedImpactPlayerUntil > Date.now()) {
        const applyGlow = () => {
    ...
    const { review_date, payload } = buildReviewPayload(); if (!review_date) {
        showToast('Enter a review date.', 'error');
        return;
    } let error; if (currentReviewId) {
    ...
    if (!confirm('Delete this review?')) return; const { error } = await reviewDelete(reviewId); if (error) {
        console.error(error);
    ...
    const remainingReviews = remainingReviewsResult?.data || []; const stillQualifiesForAutoImpact = remainingReviews.some(row => {
        const scores = [
        ...
        ].map(v => Number(v)).filter(v => !isNaN(v)); const avg = scores.length
            ? scores.reduce((sum, v) => sum + v, 0) / scores.length
            : null; return avg !== null && avg >= 3.5;
        ...
        await supabaseClient
            ...
            .eq('employee_id', employeeId)
            .eq('note_type', 'Impact Player Flag')
            .eq('note_text', 'Auto-flagged from high performance review'); const impactKey = String(employeeId);
        if (currentImpactPlayerRosterMap && currentImpactPlayerRosterMap[impactKey]) {
            ...
            const isOnlyAutoFlag = !existing.manualReason || existing.manualReason === 'Auto-flagged from high performance review'; if (isOnlyAutoFlag) {
    ...
    switchTab('reviews');
    if (typeof loadSummaryMetrics === 'function') await loadSummaryMetrics(); const impactKey = String(employeeId);
    ...
    if (!target) return; const { data, error } = await reviewGetAll(actualEmployeeId); if (error) {
        console.error(error);
        ...
        const currentScores = [
            ...
            row.attitude_score,
            row.reliability_score
        ].map(v => Number(v)).filter(v => !isNaN(v)); const currentAvg = currentScores.length
            ? currentScores.reduce((sum, v) => sum + v, 0) / currentScores.length
            : null; const previousRow = data[index + 1] || null;
        ...
        ].map(v => Number(v)).filter(v => !isNaN(v)) : []; const previousAvg = previousScores.length
        ...
        const promotionSignal = currentAvg !== null && currentAvg >= 3.5 && tenureMonths > 3; return `
        <div class="history-item">
          ...
          <div class="history-body">
            ...
            ${(() => {
                ...
                else if (score === 1) label = 'At Risk'; return `<strong>Blended Professional Score:</strong> ${esc(row.attitude_score || row.attitude || '')}${label ? ` (${esc(label)})` : ''}<br>`;
            })()}
    ...
    `;
    }).join(''); target.querySelectorAll('[data-edit-review-id]').forEach(btn => {
