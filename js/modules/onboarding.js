...
let resolvedEmployeeId = employeeId || currentEmployee?.dbId || currentEmployee?.id || selectedEmployee?.dbId || selectedEmployee?.id; if (!resolvedEmployeeId) {
    console.warn('Default onboarding tasks not created because no employee ID was found.');
    return;
} let employeeRecord = null;
let employeeLookupError = null; try {
...
} const hireDate = employeeRecord?.hire_date || new Date().toISOString().slice(0, 10); function addDaysToHireDate(daysToAdd) {
    const date = new Date(`${hireDate}T00:00:00`);
    date.setDate(date.getDate() + daysToAdd);
    return date.toISOString().slice(0, 10);
} const today = new Date().toISOString().slice(0, 10); function getStatusForDueDate(daysToAdd) {
    const dueDate = addDaysToHireDate(daysToAdd); if (dueDate === today) return 'Due Today';
    if (dueDate < today) return 'Overdue'; return 'Pending';
} const defaultTasks = [
    {
...
]; const { data: existingTasks, error: fetchError } = await supabaseClient.from('onboarding_tasks').select('id, employee_id, task_name').eq('employee_id', resolvedEmployeeId); if (fetchError) { console.error('Error fetching existing onboarding tasks:', fetchError); return; }    // Step 2: attach IDs where matches exist    const tasksWithIds = defaultTasks.map(task => {
const match = existingTasks?.find(t =>
    String(t.employee_id) === String(task.employee_id) &&
    t.task_name === task.task_name); return match ? { ...task, id: match.id } : { ...task }; // let DB handle insert without forcing id    });    const { error: upsertError } = await supabaseClient
        .from('onboarding_tasks')
    .upsert(tasksWithIds, {
        onConflict: 'employee_id,task_name',
        ignoreDuplicates: true
    }); if (upsertError) { console.error('Error creating default onboarding tasks:', upsertError); }
...
async function createDefaultOnboardingTasks(employeeId) {
    ...
    const tasksWithIds = defaultTasks.map(task => {
        const match = existingTasks?.find(existingTask =>
            String(existingTask.employee_id) === String(task.employee_id) &&
