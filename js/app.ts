declare function showToast(message: string, type?: string): void; declare const supabaseClient: any; declare function openDrawer(employee: any): void; declare function loadEmployeeReviews(id: string): Promise<void>; declare function setText(id: string, value: any): void;
...
declare const OrbisServices: any; function safeGet(id: string): any {
...
declare function renderEmployeeRoster(): void; const OrbisState = { atRiskMap: {} as Record<string, RosterRiskMeta>, impactPlayerMap: {} as Record<string, RosterRiskMeta> }; type Employee = { id: string; dbId?: string; first?: string; last?: string; first_name?: string; last_name?: string; department?: string; dept?: string; employee_id?: string; displayId?: string; payType?: string; pay_type?: string; nextReview?: Date | string | null; next_review_date?: string; tenureMonths?: number; tenure_months?: number; position?: string; supervisor?: string; status?: string; hire_date?: string;[key: string]: any; }; type UserRole = 'admin' | 'supervisor' | 'user'; type UserAccess = {
    email: string;
    display_name?: string;
    role: UserRole;
    ...
    if (currentDateEl) {
        ...
        th.addEventListener('click', () => {
            ...
            if (!nextColumn) return; if (currentSort.column === nextColumn) {
    ...
    safeGet('statusFilter')?.addEventListener('change', renderRoster); if (typeof bindDrawerEvents === 'function') {
        bindDrawerEvents();
...
let currentManualImpactPlayerState: ManualFlagState = { flagged: false, reason: '' }; (window as any).OrbisState.atRiskMap = OrbisState.atRiskMap;
...
function switchTab(tabName: string): void {
    ...
    resetDrawerForms();
    switchTab('employee'); setText('drawerTitle', 'New Employee');
    setText('drawerSub', 'Create employee record');
    if (typeof resetEmployeeForm === 'function') resetEmployeeForm(); if (safeGet('saveEmployeeBtn')) safeGet('saveEmployeeBtn').textContent = 'Save Employee'; if (safeGet('notesHistory')) safeGet('notesHistory').innerHTML = '<div class="empty">Save the employee before adding notes.</div>';
    ...
    if (safeGet('onboardingProgressBar')) safeGet('onboardingProgressBar').style.width = '0%'; const backdrop = safeGet('drawerBackdrop');
    const drawer = safeGet('employeeDrawer');
    if (backdrop) backdrop.classList.add('open');
    if (drawer) drawer.classList.add('open');
    applyRolePermissions();
} async function deleteEmployeeById(employeeId: any): Promise<{ error: any }> {
    const db = window.supabaseClient || supabaseClient; if (!db) {
        return { error: new Error('Supabase client not available') };
    } const targetId = String(employeeId || '').trim(); if (!targetId) {
    ...
    const relatedTables = [
        ...
        const { error } = await db
            .from(table)
            .delete()
            .eq('employee_id', targetId); if (error) {
                console.warn(`Could not delete related rows from ${table}:`, error);
            }
    } const { error } = await db
        .from('employees')
        .delete()
        .eq('id', targetId); if (error) {
            console.error('Employee delete failed:', error);
            return { error };
        } return { error: null };
} window.deleteEmployeeById = deleteEmployeeById; async function runDeleteEmployee(): Promise<void> {
    if (!currentEmployee) {
        showToast('Open an employee first.', 'error');
        return;
    } const employeeName = `${currentEmployee.first || currentEmployee.first_name || ''} ${currentEmployee.last || currentEmployee.last_name || ''}`.trim() || 'this employee'; const confirmed = confirm(`Permanently delete ${employeeName}'s employee file? This removes the record completely and cannot be undone.`);
    if (!confirmed) return; const employeeId = currentEmployee.id || currentEmployee.employee_id || currentEmployee.dbId;
    const deleteResult = await deleteEmployeeById(employeeId);
    const error = deleteResult?.error; if (error) {
    ...
    showToast('Employee deleted permanently.', 'success'); await loadEmployees();
    await loadSummaryMetrics();
    await loadReviewDashboard(); if (typeof closeDrawer === 'function') {
        closeDrawer();
    }
    ...
    if (!currentEmployee) {
        showToast('Open an employee first.', 'error');
        return;
    } const employeeName = `${currentEmployee.first || currentEmployee.first_name || ''} ${currentEmployee.last || currentEmployee.last_name || ''}`.trim() || 'this employee'; const confirmed = confirm(`Terminate ${employeeName}? This will mark them as TERMINATED but keep their file.`);
    if (!confirmed) return; const employeeId = currentEmployee.id || currentEmployee.employee_id || currentEmployee.dbId;
    const targetId = String(employeeId || '').trim();
    const isUuid = /^[0-9a-f-]{36}$/i.test(targetId); let query = supabaseClient
        .from('employees')
        .update({
            ...
            notes: currentEmployee.notes
                ...
                : 'Terminated employee file retained for turnover history.'
        }); query = query.eq('id', targetId); const { error } = await query; if (error) {
            console.error(error);
    ...
    showToast('Employee terminated. File retained for turnover reporting.', 'success'); await loadEmployees();
    await loadSummaryMetrics();
    await loadReviewDashboard(); if (typeof closeDrawer === 'function') {
        closeDrawer();
    }
} window.runTerminateEmployee = runTerminateEmployee; async function updateEmployeeById(employeeId: any, payload: any): Promise<{ data: any; error: any }> {
    const db = window.supabaseClient || supabaseClient; if (!db) {
        return { data: null, error: new Error('Supabase client not available') };
    } const targetId = String(employeeId || payload?.id || payload?.employee_id || payload?.dbId || '').trim(); if (!targetId) {
    ...
    delete cleanPayload.dept; const { data, error } = await db
        .from('employees')
        .update(cleanPayload)
        .eq('id', targetId)
        .select(); if (error) {
...
function resetDrawerForms(): void {
    ...
    if (safeGet('noteText')) safeGet('noteText').value = ''; if (safeGet('disciplineDate')) safeGet('disciplineDate').value = todayInputValue();
    ...
    if (safeGet('disciplineStatus')) safeGet('disciplineStatus').value = 'Open'; if (safeGet('incidentDate')) safeGet('incidentDate').value = todayInputValue();
    ...
    if (safeGet('incidentStatus')) safeGet('incidentStatus').value = 'Open'; if (safeGet('meetingDate')) safeGet('meetingDate').value = todayInputValue();
    if (safeGet('meetingType')) safeGet('meetingType').value = '';
    if (safeGet('meetingSubject')) safeGet('meetingSubject').value = '';
    if (safeGet('meetingNotes')) safeGet('meetingNotes').value = ''; if (safeGet('reviewDate')) safeGet('reviewDate').value = todayInputValue();
    ...
    if (safeGet('reviewManagerComments')) safeGet('reviewManagerComments').value = ''; if (safeGet('stayInterviewDate')) safeGet('stayInterviewDate').value = todayInputValue();
    ...
    if (safeGet('stayManagerSummary')) safeGet('stayManagerSummary').value = ''; if (safeGet('ecName')) safeGet('ecName').value = '';
    ...
    currentManualImpactPlayerState = { flagged: false, reason: '' }; currentDisciplineReportId = null;
    ...
    isCreatingEmployee = false; if (safeGet('saveDisciplineBtn')) safeGet('saveDisciplineBtn').textContent = 'Save Discipline Report';
    ...
    if (safeGet('saveReviewBtn')) safeGet('saveReviewBtn').textContent = 'Save Review'; safeGet('cancelDisciplineEditBtn')?.classList.add('hidden');
...
} function normalizeEmployee(employee: any): Employee | null { if (!employee) return null; const first = employee.first || employee.first_name || ''; const last = employee.last || employee.last_name || ''; const dept = employee.dept || employee.department || ''; const status = String(employee.status || 'ACTIVE').toUpperCase(); const hireDateRaw = employee.hire_date || employee.hireDate || ''; const nextReviewRaw = employee.next_review_date || employee.nextReviewDate || ''; const hireDate = hireDateRaw ? new Date(`${hireDateRaw}T00:00:00`) : null; const nextReview = nextReviewRaw ? new Date(`${nextReviewRaw}T00:00:00`) : null; return { ...employee, id: employee.id || employee.employee_id || '', dbId: employee.id || '', employee_id: employee.employee_id || employee.id || '', first, last, first_name: first, last_name: last, displayName: `${first} ${last}`.trim(), dept, department: dept, position: employee.position || '', supervisor: employee.supervisor || '', status, displayStatus: status, payType: employee.payType || employee.pay_type || '', pay_type: employee.pay_type || employee.payType || '', hireDate, hire_date: hireDateRaw, nextReview, next_review_date: nextReviewRaw, tenureMonths: Number(employee.tenureMonths || employee.tenure_months || 0), tenure_months: Number(employee.tenure_months || employee.tenureMonths || 0), benefitsStatus: employee.benefitsStatus || employee.benefits_status || '', benefits_status: employee.benefits_status || employee.benefitsStatus || '' }; } function populateEmployeeAdminForm(employee: any): void {
    if (!employee) return;
    employee = normalizeEmployee(employee);
    if (!employee) return; const drawerTitleName = String(safeGet('drawerTitle')?.textContent || '').trim();
    ...
    const nameParts = String(fallbackName).trim().split(/\s+/).filter(Boolean); const values = {
    ...
    setByPlaceholder('Employee ID', values.employeeId); setField('empStatus', values.status);
    setField('status', values.status); setField('empFirstName', values.firstName);
    setField('firstName', values.firstName);
    setField('employeeFirstName', values.firstName);
    setByPlaceholder('First name', values.firstName); setField('empLastName', values.lastName);
    setField('lastName', values.lastName);
    setField('employeeLastName', values.lastName);
    setByPlaceholder('Last name', values.lastName); setField('empDepartment', values.department);
    setField('department', values.department);
    setField('employeeDepartment', values.department);
    setByPlaceholder('Department', values.department); setField('empPosition', values.position);
    setField('position', values.position);
    setField('employeePosition', values.position);
    setByPlaceholder('Position', values.position); setField('empSupervisor', values.supervisor);
    setField('supervisor', values.supervisor);
    setByPlaceholder('Supervisor', values.supervisor); setField('empPayType', values.payType);
    setField('payType', values.payType);
    setByPlaceholder('Hourly, Salary, etc.', values.payType); setField('empStandardHours', values.standardHours);
    setField('standardHours', values.standardHours);
    setByPlaceholder('40', values.standardHours); setField('empBenefitsStatus', values.benefitsStatus);
    setField('benefitsStatus', values.benefitsStatus);
    setByPlaceholder('Benefits status', values.benefitsStatus); setField('empHireDate', values.hireDate);
    ...
    setField('tenureBracket', values.tenureBracket); setField('empWorkEmail', values.workEmail);
    ...
    setField('notes', values.notes); const drawer = safeGet('employeeDrawer') || document.querySelector('#employeeDrawer'); const statusSelect: any =
        safeGet('empStatus') ||
        safeGet('status') ||
        ...
        Array.from(drawer?.querySelectorAll('select') || []).find((select: any) => {
            ...
            }); statusSelect.value = matchingOption ? (matchingOption as HTMLOptionElement).value : 'ACTIVE';
...
safeGet('stayInterviewEditStatus')?.classList.add('hidden');
...
} let isLoadingDashboard = false; async function getUserRole(): Promise<UserRole | null> {
    try {
        const { data: { user } } = await supabaseClient.auth.getUser();
        if (!user) return null; const userEmail = String(user.email || '').trim().toLowerCase();
        currentUserAccess = null; const { data: accessRows, error: accessError } = await supabaseClient
            .from('user_access')
            .select('email, display_name, role, supervisor_name, can_delete')
            .eq('email', userEmail)
            .limit(1); if (!accessError && accessRows && accessRows[0]) {
                ...
                .eq('id', user.id); if (error) {
                    ...
                    .filter(Boolean); if (roles.includes('admin')) return 'admin';
    ...
    if (!isSupervisorUser()) return true; const supervisorName = String(currentUserAccess?.supervisor_name || '').trim().toLowerCase();
    if (!supervisorName) {
    ...
    const compactEmployeeSupervisor = employeeSupervisor.replace(/[^a-z0-9]/g, ''); const isMatch = employeeSupervisor.includes(supervisorName) ||
        supervisorName.includes(employeeSupervisor) ||
        compactEmployeeSupervisor.includes(compactAccessName) ||
        compactAccessName.includes(compactEmployeeSupervisor); console.log('[Supervisor Match Check]', {
            employeeSupervisor,
            supervisorName,
            isMatch
        }); return isMatch;
} function applySupervisorDashboardView(): void {
    if (!isSupervisorUser()) return; const name = currentUserAccess?.display_name || currentUserAccess?.supervisor_name || 'Supervisor'; const title = safeGet('dashboardTitle') || document.querySelector('h1');
    if (title) title.textContent = `${name}'s Team Dashboard`; const rosterTitle = safeGet('rosterTitle') || document.querySelector('#employeeRosterCard h2, #employeeRosterCard h3, .roster-title');
    if (rosterTitle) rosterTitle.textContent = 'My Team';    // KPI LABEL UPDATES
    const activeLabel = document.querySelector('#kActiveHC')?.closest('.kpi-card')?.querySelector('.kpi-label');
    if (activeLabel) activeLabel.textContent = 'My Team Size'; const reviewsLabel = document.querySelector('#kReviewsDue')?.closest('.kpi-card')?.querySelector('.kpi-label');
    if (reviewsLabel) reviewsLabel.textContent = 'My Reviews Due'; const riskLabel = document.querySelector('#kTurnoverRisk')?.closest('.kpi-card')?.querySelector('.kpi-label');
    if (riskLabel) riskLabel.textContent = 'My Team Risk'; const leaveLabel = document.querySelector('#kOnLeave')?.closest('.kpi-card')?.querySelector('.kpi-label');
    if (leaveLabel) leaveLabel.textContent = 'My Team On Leave';    // HIDE COMPANY-WIDE KPIs
    const deptCard = document.querySelector('#kDepartments')?.closest('.kpi-card');
    if (deptCard) deptCard.classList.add('hidden');    // ADMIN UI LOCK
    document.querySelectorAll('[data-admin-only="true"], .admin-only').forEach(el => { const control = el as HTMLElement & { disabled?: boolean }; control.classList.add('hidden'); control.disabled = true; });    // OPTIONAL: add simple supervisor insight banner
    ...
    if (!existingBanner) {
        ...
        banner.style.fontSize = '14px'; const atRisk = (window.EMPLOYEES || []).filter(e => {
        ...
        }).length; banner.textContent = `You have ${EMPLOYEES.length} employees. ${atRisk} may need attention.`; const container = document.querySelector('.dashboard') || document.body;
        if (container) container.prepend(banner);
    }
} function getAuditTrail(): any[] { try { const raw = localStorage.getItem('btw_hris_audit_trail'); const parsed = raw ? JSON.parse(raw) : []; return Array.isArray(parsed) ? parsed : []; } catch (_err) { return []; } } function buildEmployeeChangeLog(oldEmployee: any, newEmployee: any): string { const oldData = normalizeEmployee(oldEmployee || {}); const newData = normalizeEmployee(newEmployee || {}); const fields = [['Status', 'status'], ['First Name', 'first_name'], ['Last Name', 'last_name'], ['Department', 'department'], ['Position', 'position'], ['Supervisor', 'supervisor'], ['Pay Type', 'pay_type'], ['Standard Hours', 'standard_hours'], ['Benefits Status', 'benefits_status'], ['Hire Date', 'hire_date'], ['Next Review Date', 'next_review_date'], ['Anniversary Date', 'anniversary_date'], ['Tenure Bracket', 'tenure_bracket'], ['Work Email', 'work_email'], ['Personal Email', 'personal_email'], ['Phone', 'phone']]; const formatValue = (value: any): string => { const text = String(value ?? '').trim(); return text || 'Blank'; }; return fields.map(([label, key]) => { const oldValue = formatValue(oldData?.[key]); const newValue = formatValue(newData?.[key]); return oldValue !== newValue ? `${label}: ${oldValue} → ${newValue}` : ''; }).filter(Boolean).join(' | '); } function recordAuditEvent(action: string, employee: any, details: string = ''): void {
    try {
        const audit = getAuditTrail();        // Prevent useless or empty audit entries like "Blank → Blank"
        const cleanDetails = String(details || '').trim(); if (!cleanDetails || cleanDetails === 'Blank → Blank') {
            console.warn('Skipped empty audit log entry.');
            return;
        } const entry = { action, employeeId: employee?.id || employee?.dbId || '', employeeName: employee ? `${employee.first || ''} ${employee.last || ''}`.trim() : '', details: cleanDetails, userRole: currentUserRole || 'user', timestamp: new Date().toISOString() }; audit.unshift(entry); localStorage.setItem('btw_hris_audit_trail', JSON.stringify(audit.slice(0, 75)));
    } catch (err) { console.error('Could not write audit trail.', err); }
} function applyRoleLocks(): void {
    const adminOnlyIds = [
        'deleteEmployeeBtn',
        'terminateEmployeeBtn'
    ]; adminOnlyIds.forEach(id => { const el = safeGet(id); if (!el) return; const locked = !canManageEmployeeRecords(); el.disabled = locked; el.title = locked ? 'Locked: admin access required' : ''; });
    ...
    const terminateBtn = safeGet('terminateEmployeeBtn'); if (currentEmployee) {
        const status = String(currentEmployee.status || '').toUpperCase(); if (deleteEmployeeBtn) {
    ...
    if (deleteECBtn) {
        ...
        ).forEach(el => { const control = el as HTMLElement & { disabled?: boolean }; control.classList.add('hidden'); control.disabled = true; control.title = 'Locked: supervisors cannot delete or terminate records'; });
    } if (supervisorMode) {
    ...
    const searchRoot = drawer || document; const findButtonByText = (labels: string[]): any => {
        ...
        findButtonByText(['New Employee']); const saveBtn =
            (drawer && drawer.querySelector('#saveEmployeeBtn')) ||
            findButtonByText(['Update Employee', 'Save Employee']); const actionsRow =
                ...
                drawer; if (!actionsRow) { console.warn('Could not find employee action row for Archive/Terminate buttons.'); return null; } let archiveBtn = safeGet('deleteEmployeeBtn'); if (!archiveBtn) { archiveBtn = document.createElement('button'); archiveBtn.type = 'button'; archiveBtn.id = 'deleteEmployeeBtn'; archiveBtn.className = 'button danger'; archiveBtn.textContent = 'Delete Employee'; archiveBtn.onclick = () => runDeleteEmployee(); } let terminateBtn = safeGet('terminateEmployeeBtn'); if (!terminateBtn) { terminateBtn = document.createElement('button'); terminateBtn.type = 'button'; terminateBtn.id = 'terminateEmployeeBtn'; terminateBtn.className = 'button danger'; terminateBtn.textContent = 'Terminate Employee'; terminateBtn.onclick = () => runTerminateEmployee(); } if (!actionsRow.contains(archiveBtn)) { if (newBtn && newBtn.nextSibling) { actionsRow.insertBefore(archiveBtn, newBtn.nextSibling); } else { actionsRow.appendChild(archiveBtn); } } if (!actionsRow.contains(terminateBtn)) {
                ...
                } archiveBtn.classList.remove('hidden'); terminateBtn.classList.remove('hidden'); applyRoleLocks(); return archiveBtn;
} async function loadAllDashboardData(): Promise<void> {
    if (isLoadingDashboard) return;
    isLoadingDashboard = true; try {
        await Promise.all([
            loadEmployees(),
            loadCandidates(),
            loadSummaryMetrics(),
    ...
    try {
        const { data: { user } } = await supabaseClient.auth.getUser();
        const userEmail = String(user?.email || '').trim().toLowerCase(); if (userEmail) {
        ...
        .select('*'); if (error) {
            ...
            .filter(Boolean); window.ALL_EMPLOYEES = normalizedEmployees; if (isSupervisorUser()) {
                if (!currentUserAccess?.supervisor_name) { showToast('No employee access assigned. Contact HR.', 'error'); EMPLOYEES = []; } else { EMPLOYEES = normalizedEmployees.filter(employeeMatchesSupervisorAccess); } console.log('[Supervisor Filter Applied]', {
    ...
    console.log('[Access Scope]', currentUserRole, currentUserAccess, 'visible employees:', EMPLOYEES.length); if (typeof renderEmployeeRoster === 'function') {
...
async function loadCandidates(): Promise<void> {
...
} function buildCandidateInterviewNotice(candidate: any): string { const status = String(candidate?.interview_status || '').toLowerCase(); if (status === 'cancelled' || status === 'completed' || status === 'no show') return ''; if (!candidate?.interview_date || !candidate?.interview_time) return ''; const interviewDateTime = new Date(`${candidate.interview_date}T${candidate.interview_time}`); const now = new Date(); if (Number.isNaN(interviewDateTime.getTime())) return ''; if (interviewDateTime <= now) return ''; const date = interviewDateTime.toLocaleDateString(); const time = interviewDateTime.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }); const type = candidate.interview_type ? ` • ${candidate.interview_type}` : ''; return `Upcoming Interview: ${date} at ${time}${type}`; } function renderCandidates(): void {
    const pipeline = safeGet('candidatePipeline');
    const tableBody = safeGet('candidateBody'); const stages = ['Applied', 'Screening', 'Interviewing', 'Offer'];
    setText('candidateCount', `${CANDIDATES.length} candidate${CANDIDATES.length === 1 ? '' : 's'}`); if (tableBody) {
        if (!CANDIDATES.length) {
            ...
            tableBody.innerHTML = CANDIDATES.map(candidate => {
                ...
                `;
            }).join(''); tableBody.querySelectorAll('[data-candidate-id]').forEach(el => {
                el.addEventListener('click', async event => {
                    event.stopPropagation(); if (typeof window.openCandidateDetails === 'function') {
    ...
    }, 0); return `BTW${maxExisting + 1}`;
} async function generateAvailableEmployeeId(): Promise<string> {
    const usedNumbers = new Set(); const collectNumber = (value: any): void => {
    ...
    } let nextNumber = usedNumbers.size ? Math.max(...Array.from(usedNumbers as Set<number>)) + 1 : 1; while (usedNumbers.has(nextNumber)) {
        nextNumber += 1;
    } return `BTW${nextNumber}`;
} async function createDefaultOnboardingTasks(employeeId: any): Promise<void> {
    if (!employeeId) return; const defaultTasks = [
        'Complete I-9',
        'Complete W-4',
        ...
        .from('onboarding_tasks')
        .select('task_name')
        .eq('employee_id', employeeId); if (existingError) {
            console.warn('Could not check existing onboarding tasks:', existingError);
        } const existingTaskNames = new Set((existingTasks || []).map(task => String(task.task_name || '').trim())); const payload = defaultTasks
            .filter(taskName => !existingTaskNames.has(taskName))
            .map(taskName => ({
                employee_id: employeeId,
                task_name: taskName,
                status: 'Pending'
            })); if (!payload.length) return; const { error } = await supabaseClient
                .from('onboarding_tasks')
                .insert(payload); if (error) {
    ...
    if (!employeeId) return; let { data, error } = await supabaseClient
        .from('onboarding_tasks')
        .select('*')
        .eq('employee_id', employeeId)
        .order('task_name', { ascending: true }); if (error) {
    ...
    if (!data || data.length === 0) {
        await createDefaultOnboardingTasks(employeeId); const retry = await supabaseClient
            .from('onboarding_tasks')
            .select('*')
            .eq('employee_id', employeeId)
            .order('task_name', { ascending: true }); if (retry.error) {
                console.error('Could not reload onboarding tasks:', retry.error);
                return;
            } data = retry.data || [];
    } const tasks = data || []; const container = document.getElementById('onboardingChecklist');
    const summary = document.getElementById('onboardingSummary');
    const bar = document.getElementById('onboardingProgressBar'); if (!container) return; if (!tasks.length) {
    ...
    const percent = Math.round((completed / tasks.length) * 100); container.innerHTML = tasks.map(task => `
    ...
    `).join(''); if (summary) summary.textContent = `${completed} of ${tasks.length} complete`;
    if (bar) bar.style.width = `${percent}%`;
} async function toggleOnboardingTask(taskId: any, isComplete: boolean): Promise<void> {
    if (!taskId) return; const { error } = await supabaseClient
        .from('onboarding_tasks')
        .update({ status: isComplete ? 'Completed' : 'Pending' })
        .eq('id', taskId); if (error) {
...
window.toggleOnboardingTask = toggleOnboardingTask; async function convertCandidateToEmployee(candidateId: any): Promise<void> {
    ...
    if (!candidate) {
        ...
        return;
    } candidate.__isConvertingToEmployee = true; const conversionButtons = Array.from(document.querySelectorAll('button'))
        .filter(button => {
        ...
        }); let newEmployeeId = await generateAvailableEmployeeId(); const payload = {
            id: newEmployeeId,
    ...
    let error = null; const result = await supabaseClient
        .from('employees')
        .insert([{ ...payload, id: newEmployeeId }])
        .select(); data = result.data;
    error = result.error; if (error) {
        console.error(error);
        showToast(error.message || 'Could not convert candidate to employee.', 'error'); candidate.__isConvertingToEmployee = false;
        conversionButtons.forEach(button => {
    ...
    if (!Array.isArray(window.EMPLOYEES)) {
        ...
        const normalizedNewEmployee = typeof normalizeEmployee === 'function'
            ...
            : data[0]; const exists = window.EMPLOYEES.some(e =>
    ...
    if (typeof renderEmployeeRoster === 'function') {
        ...
        .from('candidates')
        .update({ stage: 'Hired' })
        .eq('id', candidate.id); if (candidateError) {
    ...
    await loadReviewDashboard(); const refreshedEmployee = EMPLOYEES.find(e => String(e.employee_id || e.id) === String(newEmployeeId));
    ...
    const backdrop = safeGet('drawerBackdrop');
    const drawer = safeGet('candidateDrawer'); if (backdrop) backdrop.classList.add('open');
    if (drawer) {
    ...
    isCreatingCandidate = false;
    resetCandidateForm(); if (safeGet('candidateFirstNameInput')) safeGet('candidateFirstNameInput').value = candidate.first_name || '';
    ...
    await loadCandidates(); const refreshedCandidate = CANDIDATES.find(item =>
    ...
    if (!currentCandidate) {
        ...
        .from('candidates')
        .delete()
        .eq('id', currentCandidate.id); if (error) {
            console.error(error);
...
window.convertCurrentCandidateToEmployee = convertCurrentCandidateToEmployee; function populateDepartmentFilter(): void {
    const deptSelect = safeGet('deptFilter');
    if (!deptSelect) return; const currentValue = deptSelect.value; const depts = [...new Set(
        EMPLOYEES.map(e => e.dept).filter(Boolean)
    )].sort(compareText); deptSelect.innerHTML =
        '<option value="">All Departments</option>' +
        depts.map(d => `<option value="${esc(d)}">${esc(d)}</option>`).join(''); deptSelect.value = currentValue;
} function openCandidatesView(): void {
    const el = document.getElementById('candidatesCard'); if (!el) return;
    el.scrollIntoView({ behavior: 'smooth', block: 'start' }); el.style.boxShadow = '0 0 0 3px rgba(59,130,246,0.25)';
    setTimeout(() => {
        el.style.boxShadow = '';
    }, 1500);
} window.openCandidatesView = openCandidatesView; function renderRoster(): void {
    if (typeof renderEmployeeRoster === 'function') {
        renderEmployeeRoster();
        return;
    }
    ...
    if (!body) return; const counts = {};
    EMPLOYEES.filter(e => e.status === 'ACTIVE').forEach(e => {
        const dept = e.dept || 'Unassigned';
        counts[dept] = (counts[dept] || 0) + 1;
    }); const rows = Object.entries(counts).sort((a, b) => compareText(a[0], b[0])); if (!rows.length) {
    ...
    const today = new Date();
    today.setHours(0, 0, 0, 0); const overdueReviewEmployees = reviewEligibleActive.filter(e => {
    ...
    const reviewsDue = overdueReviewEmployees.length; const turnoverRiskEmployees = reviewEligibleActive.filter(e => {
    ...
    const turnoverRisk = reviewEligibleActive.length
        ...
        : 0; setText('kActiveHC', active.length);
    ...
    if (reviewsDueInfo) {
        const overdueNames = overdueReviewEmployees
            ...
            .filter(Boolean)
            .sort((a, b) => compareText(a, b)); reviewsDueInfo.title = overdueNames.length
    ...
    try {
        const [disciplineRes, reviewsRes, incidentsRes, manualRiskRes, impactPlayerRes] = await Promise.all([
            ...
            setText('kOpenDiscipline', openCount); const disciplineCard = document.getElementById('cardOpenDiscipline');
            if (disciplineCard) {
                const openDisciplineNames = openDisciplineCases
                    .map(row => {
                        ...
                        const issueType = String(row?.issue_type || '').trim(); if (fullName && issueType) return `${fullName} (${issueType})`;
                        if (fullName) return fullName;
                        if (issueType) return issueType;
                        return 'Unnamed discipline case';
                    })
                    .filter(Boolean); const disciplineTooltip = openDisciplineNames.length
                        ? openDisciplineNames.join('\n')
                        : 'No open discipline cases'; disciplineCard.setAttribute('data-tooltip', disciplineTooltip);
                disciplineCard.removeAttribute('title');
            }
        } else {
            console.error(disciplineRes.error);
            setText('kOpenDiscipline', '—'); const disciplineCard = document.getElementById('cardOpenDiscipline');
            if (disciplineCard) {
        ...
        const impactPlayerEmployeeIds = new Set(); if (!reviewsRes.error) {
            (reviewsRes.data || []).forEach(row => {
                const employeeId = row.employee_id;
                const sortDate = row.review_date || row.created_at || '';
                if (!employeeId) return; if (!latestReviewByEmployee[employeeId] || String(sortDate) > String(latestReviewByEmployee[employeeId].sortDate)) {
                    const scoreValues = [
                    ...
                    ].filter(v => v !== null && v !== undefined && v !== ''); const avgScore = scoreValues.length
                        ? scoreValues.reduce((sum, v) => sum + Number(v), 0) / scoreValues.length
                        : null; latestReviewByEmployee[employeeId] = { avgScore, sortDate };
                }
            }); Object.entries(latestReviewByEmployee).forEach(([employeeId, item]: [string, any]) => {
        ...
        applyReviewImpactPlayers(latestReviewByEmployee); Object.keys(OrbisState.impactPlayerMap || {}).forEach(key => {
            const meta = OrbisState.impactPlayerMap[key];
            const hasManualFlag = !!(meta?.manualReason && String(meta.manualReason).trim() !== '');
            const hasReviewFlag = meta?.highReview === true; if (!hasManualFlag && !hasReviewFlag) {
        ...
        const hasAnyData = !reviewsRes.error || !incidentsRes.error; if (hasAnyData) {
            ...
            const impactSubEl = safeGet('kImpactPlayersSub'); if (impactValueEl) {
...
async function loadReviewDashboard() {
    ...
    if (!summaryTarget || !bodyTarget) return; const activeEmployees = EMPLOYEES.filter(e => e.status === 'ACTIVE' && !String(e.payType || '').toLowerCase().includes('contract'));
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const next30 = new Date(today);
    next30.setDate(next30.getDate() + 30);
    const last30 = new Date(today);
    last30.setDate(last30.getDate() - 30); try {
        ...
        const error = meetingsRes.error; if (error) {
            console.error(error);
        ...
        if (!reviewsRes.error) {
            ...
            reviewDate.setHours(0, 0, 0, 0);
            return reviewDate <= today;
        }).length; const dueSoonReviews = activeEmployees.filter(e => {
            if (!e.nextReview || !(e.nextReview instanceof Date) || isNaN(e.nextReview.getTime())) return false;
            const reviewDate = new Date(e.nextReview);
            reviewDate.setHours(0, 0, 0, 0);
            return reviewDate > today && reviewDate <= next30;
        }).length; const completedLast30Days = reviewMeetings.filter(row => {
        ...
        }).length; summaryTarget.innerHTML = `
        ...
        `; const reviewRows = activeEmployees
            .map(employee => {
                ...
                let statusClass = 'badge badge-soft'; if (employee.nextReview && employee.nextReview instanceof Date && !isNaN(employee.nextReview.getTime())) {
                    const reviewDate = new Date(employee.nextReview);
                    reviewDate.setHours(0, 0, 0, 0); if (reviewDate <= today) {
                        statusLabel = 'Overdue';
            ...
            .slice(0, 12); if (!reviewRows.length) {
        ...
        `).join(''); bodyTarget.querySelectorAll('[data-review-employee-id]').forEach(el => {
    ...
    if (!target) return; const activeEmployees = EMPLOYEES.filter(e => e.status === 'ACTIVE' && !String(e.payType || '').toLowerCase().includes('contract'));
    const today = new Date();
    today.setHours(0, 0, 0, 0); try {
        const { data, error } = await supabaseClient
            .from('employee_reviews')
            .select('employee_id, attendance_score, performance_score, teamwork_score, attitude_score, reliability_score, overall_result, created_at, review_date')
            .order('review_date', { ascending: false }); if (error) {
                console.error(error);
        ...
        (data || []).forEach(row => {
            ...
            if (!employeeId) return; if (!latestReviewByEmployee[employeeId] || String(sortDate) > String(latestReviewByEmployee[employeeId].sortDate)) {
                const scoreValues = [
                    ...
                    ? scoreValues.reduce((sum, v) => sum + Number(v), 0) / scoreValues.length
                    : null; latestReviewByEmployee[employeeId] = {
                        avgScore,
        ...
        }); let insightText = 'All core HR indicators are stable today.'; if (overdueReviewEmployees.length || lowPerformers.length || earlyTenureEmployees.length) {
    ...
    today.setHours(0, 0, 0, 0); try {
        ...
        (incidentsRes.data || []).forEach(row => {
            const employeeId = String(row.employee_id || '');
            const status = String(row.status || '').toLowerCase();
            if (!employeeId || status === 'closed') return; openIncidentCountsByEmployee[employeeId] =
        ...
        (manualRiskRes.data || []).forEach(row => {
            ...
            .map(employee => {
                ...
                const manualRisk = manualRiskByEmployee[employeeKey] || null; if (latestReview && latestReview.avgScore !== null && latestReview.avgScore <= 3) {
            ...
            .slice(0, 6); if (!atRiskEmployees.length) {
        ...
        `).join(''); target.querySelectorAll('[data-risk-employee-id]').forEach(card => {
    ...
    if (!target) return; target.innerHTML = '<div class="empty">Loading activity...</div>'; try {
...
} function setManualAtRiskUi(flagged, reason = '') { currentManualAtRiskState = { flagged: !!flagged, reason: String(reason || '').trim() }; if (safeGet('atRiskReasonInput')) { safeGet('atRiskReasonInput').value = currentManualAtRiskState.reason; } } async function loadEmployeeManualAtRisk(employeeId) { const actualEmployeeId = currentEmployee?.dbId || employeeId; if (!actualEmployeeId) { setManualAtRiskUi(false, ''); return; } const { data, error } = await supabaseClient.from('employee_notes').select('id, note_type, note_text, note_date, created_at').eq('employee_id', actualEmployeeId).in('note_type', ['At-Risk Flag', 'At-Risk Cleared']).order('created_at', { ascending: false }).limit(1); if (error) { console.error(error); setManualAtRiskUi(false, ''); return; } const latest = data?.[0]; if (!latest || latest.note_type !== 'At-Risk Flag') { setManualAtRiskUi(false, ''); return; } setManualAtRiskUi(true, latest.note_text || ''); } async function markEmployeeAtRisk() {
    if (!currentEmployee) { showToast('Open an employee first.', 'error'); return; } const reason = String(safeGet('atRiskReasonInput')?.value || '').trim(); if (!reason) { showToast('Enter a reason before marking the employee at-risk.', 'error'); return; } const employeeDbId = currentEmployee.dbId || currentEmployee.id; const { error } = await supabaseClient.from('employee_notes').insert([{ employee_id: employeeDbId, note_date: todayInputValue(), note_type: 'At-Risk Flag', note_text: reason }]); if (error) { console.error(error); showToast('Could not mark employee at-risk.', 'error'); return; } showToast('Employee marked at-risk.');
    recordAuditEvent('Marked At-Risk', currentEmployee, reason); setManualAtRiskUi(true, reason); const riskMeta = {
        manualReason: reason,
        lowReview: false,
        reviewScore: null,
        openIncidentCount: 0,
    ...
    ].filter(Boolean).map(String); (window as any).currentAtRiskRosterMap = (window as any).currentAtRiskRosterMap || OrbisState.atRiskMap || {};
    OrbisState.atRiskMap = (window as any).currentAtRiskRosterMap; riskKeys.forEach(key => {
    ...
    if (!currentEmployee) { showToast('Open an employee first.', 'error'); return; } const employeeDbId = currentEmployee.dbId || currentEmployee.id; const noteText = String(safeGet('atRiskReasonInput')?.value || '').trim() || 'Manual at-risk flag cleared'; const { error } = await supabaseClient.from('employee_notes').insert([{ employee_id: employeeDbId, note_date: todayInputValue(), note_type: 'At-Risk Cleared', note_text: noteText }]); if (error) { console.error(error); showToast('Could not clear at-risk flag.', 'error'); return; } showToast('At-risk flag cleared.');
    recordAuditEvent('Cleared At-Risk', currentEmployee, noteText); setManualAtRiskUi(false, ''); const riskKeys = [
        currentEmployee.dbId,
        currentEmployee.id,
        currentEmployee.employee_id,
        currentEmployee.displayId
    ...
    if (!reason) {
        ...
        return;
    } const employeeDbId = currentEmployee.dbId || currentEmployee.id; const { error } = await supabaseClient
        .from('employee_notes')
        .insert([{
    ...
    recordAuditEvent('Marked Impact Player', currentEmployee, reason); setManualImpactPlayerUi(true, reason); const impactMeta = {
        manualReason: reason,
        flaggedDate: todayInputValue(),
        flaggedBy: 'Matthew Zinni',
        highReview: false,
        ...
        currentEmployee.employee_id,
        currentEmployee.displayId
    ].filter(Boolean).map(String); (window as any).currentImpactPlayerRosterMap = (window as any).currentImpactPlayerRosterMap || OrbisState.impactPlayerMap || {};
    OrbisState.impactPlayerMap = (window as any).currentImpactPlayerRosterMap; impactKeys.forEach(key => {
    ...
    const noteText = String(safeGet('impactPlayerReasonInput')?.value || '').trim() || 'Manual Impact Player flag cleared'; const { error } = await supabaseClient
        .from('employee_notes')
        .insert([{
            employee_id: employeeDbId,
            note_date: todayInputValue(),
    ...
    setManualImpactPlayerUi(false, ''); const impactKeys = [
        currentEmployee.dbId,
        currentEmployee.id,
        currentEmployee.employee_id,
        currentEmployee.displayId
    ...
    isCreatingEmployee = true;
    applyRolePermissions();
    if (typeof resetEmployeeForm === 'function') resetEmployeeForm(); if (safeGet('atRiskReasonInput')) safeGet('atRiskReasonInput').value = ''; currentManualAtRiskState = { flagged: false, reason: '' }; if (safeGet('impactPlayerReasonInput')) safeGet('impactPlayerReasonInput').value = ''; currentManualImpactPlayerState = { flagged: false, reason: '' };
    ...
    const tenure_bracket = safeGet('employeeTenureBracketInput')?.value.trim() || null; if (!id || !first_name || !last_name) {
        ...
        anniversary_date,
        tenure_bracket
    }; let error; if (isCreatingEmployee || !currentEmployee) { const result = await createEmployee(payload); error = result.error; } else { const result = await updateEmployeeById(currentEmployee.dbId, payload); error = result.error; } if (error) {
        console.error(error);
    ...
    await loadSummaryMetrics();
    await loadRecentActivity();
    await loadReviewDashboard(); const savedEmployeeId = id;
    ...
    if (!confirmed) return; const employeeKey = String(currentEmployee.id || currentEmployee.dbId || ''); const { error: onboardingDeleteError } = await supabaseClient
        .from('onboarding_tasks')
        .delete()
        .eq('employee_id', employeeKey); if (onboardingDeleteError) {
    ...
    if (!target) return; const resolvedEmployeeId = currentEmployee?.dbId || employeeId; const { data, error } = await supabaseClient
        ...
        .order('created_at', { ascending: false }); if (error) {
            ...
            return;
        } const rows = data || []; if (!rows.length) {
    ...
    console.log('Deleting document id:', docId); if (!confirm('Delete this document?')) return; const { data: docRows, error: fetchError } = await supabaseClient
        .from('employee_documents')
        .select('id, file_path, employee_id')
        .eq('id', docId); console.log('Fetched document row:', docRows, fetchError); const docRow = docRows?.[0]; if (fetchError || !docRow) {
            ...
            const { data: storageData, error: storageError } = await supabaseClient
                ...
                .remove([docRow.file_path]); console.log('Storage delete result:', storageData, storageError); if (storageError) {
            ...
            .from('employee_documents')
            .delete()
            .eq('id', docId); console.log('DB delete error:', deleteError); if (deleteError) {
...
if (!currentEmployee) return; const resolvedEmployeeId = currentEmployee.dbId || currentEmployee.id;
...
const notes = safeGet('ecNotes')?.value.trim() || ''; if (!contact_name) {
    showToast('Enter the emergency contact name.', 'error');
    return;
} let error; if (currentEmergencyContactId) {
    const result = await supabaseClient
        .from('emergency_contacts')
        .update({
            contact_name,
        ...
        .eq('employee_id', resolvedEmployeeId); error = result.error;
} else {
    const result = await supabaseClient
        .from('emergency_contacts')
        .insert([{
...
const file = safeGet('docFile')?.files?.[0]; if (!document_type) {
...
const filePath = `${currentEmployee.id}/${Date.now()}_${safeName}`; const { error: uploadError } = await supabaseClient.storage
    .from('employee-documents')
    .upload(filePath, file); if (uploadError) {
        console.error(uploadError);
        showToast(`Upload failed: ${uploadError.message}`, 'error');
        return;
    } const { data: authData } = await supabaseClient.auth.getUser(); const { error: insertError } = await supabaseClient
        .from('employee_documents')
        .insert([{
...
await loadEmployeeDocuments(currentEmployee.id);
    ...
    if (!target) return; const { data, error } = await supabaseClient
        ...
        .select('*')
        .eq('employee_id', employeeId)
        .order('uploaded_at', { ascending: false }); if (error) {
            console.error(error);
            ...
            data.map(async (row) => {
                const { data: signedData, error: signedError } = await supabaseClient.storage
                    .from('employee-documents')
                    .createSignedUrl(row.file_path, 3600); return {
                        ...row,
...
`).join(''); target.querySelectorAll('[data-delete-doc-id]').forEach((btn: any) => {
...
async function loadEmployeeOnboarding(employeeId) {
    ...
    if (!target) return; target.innerHTML = '<div class="empty">Loading onboarding...</div>'; try {
        const resolvedEmployeeId = currentEmployee?.id || employeeId; const { data, error } = await supabaseClient
            .from('onboarding_tasks')
            .select('id, task_name, task_type, section, status, due_date, completed_at')
            .eq('employee_id', String(resolvedEmployeeId)); if (error) {
                console.error(error);
        ...
        const rows = []; rawRows.forEach(row => {
            const taskKey = [
                String(row.task_name || '').trim().toLowerCase(),
                String(row.task_type || '').trim().toLowerCase(),
                String(row.section || '').trim().toLowerCase(),
            ].join('|'); if (seenOnboardingTasks.has(taskKey)) return; seenOnboardingTasks.add(taskKey);
            rows.push(row);
        }); const today = new Date().toISOString().slice(0, 10); rows.sort((a, b) => {
            ...
            const bOverdue = !bDone && (String(b.status || '').toLowerCase() === 'overdue' || (b.due_date && String(b.due_date) < today)); if (aOverdue && !bOverdue) return -1;
            if (!aOverdue && bOverdue) return 1; const aDue = String(a.due_date || '9999-12-31');
        ...
        const percent = Math.round((completed / total) * 100); if (summary) summary.textContent = `${completed} of ${total} complete`;
        if (bar) bar.style.width = `${percent}%`; const grouped = {};
        rows.forEach(row => {
            ...
            const badgeClass = done ? 'badge-active' : overdue ? 'badge-inactive' : dueToday ? 'badge-leave' : 'badge-soft'; return `
    ...
    const hireDate = new Date().toISOString().split('T')[0]; const nextReviewDate = (() => {
        const date = new Date(`${hireDate}T00:00:00`);
        date.setFullYear(date.getFullYear() + 1);
        return date.toISOString().slice(0, 10);
        ...
        : await OrbisServices.employees.create(payload); if (result.error) {
            ...
            .from('candidates')
            .update({ status: 'Hired' })
            .eq('id', candidate.id); if (candidateUpdateError) {
    ...
    await loadEmployees();
} async function deleteEmergencyContact() { if (!currentEmergencyContactId) { showToast('No emergency contact to delete.', 'error'); return; } if (!confirm('Are you sure you want to delete this emergency contact?')) { return; } try { const resolvedEmployeeId = currentEmployee?.dbId || currentEmployee?.id || null; const deletingId = String(currentEmergencyContactId); const { error } = await supabaseClient.from('emergency_contacts').delete().eq('id', deletingId); if (error) { console.error(error); showToast('Could not delete emergency contact.', 'error'); return; } const { data: remainingRows, error: verifyError } = await supabaseClient.from('emergency_contacts').select('id').eq('id', deletingId).limit(1); if (verifyError) { console.error(verifyError); showToast('Could not verify emergency contact deletion.', 'error'); return; } if (remainingRows && remainingRows.length) { showToast('Emergency contact was not deleted.', 'error'); return; } showToast('Emergency contact deleted.'); resetEmergencyContactForm(); if (resolvedEmployeeId) { await loadEmergencyContacts(resolvedEmployeeId); } } catch (err) { console.error(err); showToast('Error deleting emergency contact.', 'error'); } } async function loadStayInterviews(employeeId) {
    ...
    if (!target) return; const { data, error } = await supabaseClient
      ...
      <div class="history-item">
        ...
        `;
        }).join(''); target.querySelectorAll('[data-edit-stay-id]').forEach(btn => {
...
}; let error = null; if (currentStayInterviewId) {
    const result = await supabaseClient
        .from('stay_interviews')
        .update(payload)
...
currentStayInterviewId = null;
if (safeGet('saveStayInterviewBtn')) safeGet('saveStayInterviewBtn').textContent = 'Save Stay Interview';    // ✅ Reset form after save (important for UX)
if (safeGet('stayInterviewDate')) safeGet('stayInterviewDate').value = todayInputValue();
if (safeGet('stayInterviewType')) safeGet('stayInterviewType').value = '';
if (safeGet('stayQ1')) safeGet('stayQ1').value = '';
if (safeGet('stayQ2')) safeGet('stayQ2').value = '';
...
if (safeGet('stayQ6')) safeGet('stayQ6').value = '';
if (safeGet('stayQ7')) safeGet('stayQ7').value = '';
if (safeGet('stayManagerSummary')) safeGet('stayManagerSummary').value = ''; loadStayInterviews(currentEmployee.id);
}
async function editStayInterview(stayInterviewId) {
    const { data, error } = await supabaseClient
        .from('stay_interviews')
        .select('*')
        .eq('id', stayInterviewId); const interview = data?.[0]; if (error || !interview) {
            console.error(error);
    ...
    if (!confirmed) return; const { error } = await supabaseClient
        .from('stay_interviews')
        .delete()
        .eq('id', stayInterviewId); if (error) {
            console.error(error);
    ...
    if (!target) return; try {
        const employeeList = Array.isArray(EMPLOYEES)
            ...
            : []; const impactPlayers = Object.entries(OrbisState.impactPlayerMap || {})
                ...
                .slice(0, 6); if (!impactPlayers.length) {
    ...
    `;
                }).join(''); target.querySelectorAll('[data-id]').forEach((el: any) => {
                    el.onclick = () => {
...
// =========================// KPI TOOLTIP UI// =========================let kpiTooltipEl = null;function ensureKpiTooltip() {
return null;
    ...
    const el = safeGet(id);
    if (!el) return; const cleaned = (lines || []).map(v => String(v || '').trim()).filter(Boolean);
    const text = cleaned.length ? cleaned.join('\n') : emptyText; el.removeAttribute('title');
    el.setAttribute('data-tooltip', text);
} function buildKpiHoverDetails() {
    const activeEmployees = EMPLOYEES.filter(e => String(e.status || '').toUpperCase() === 'ACTIVE');
    const leaveEmployees = EMPLOYEES.filter(e => String(e.status || '').toUpperCase() === 'LEAVE'); setCardTitle(
        'cardActiveHC',
    ...
    setCardTitle('cardDepartments', deptCounts, 'No departments available'); const riskEmployees = activeEmployees
        .filter(e => {
            const tenureMonths = Number(e.tenureMonths || 0);
            const isFirstThreeMonths = tenureMonths > 0 && tenureMonths <= 3;
            const employeeKey = String(e.dbId || e.id || '');
        ...
        .filter(Boolean);
    setCardTitle('cardTurnoverRisk', riskEmployees, 'No at-risk employees in their first 3 months'); const atRiskNames = activeEmployees
        .filter(employee => {
            const keys = [
                employee.dbId,
                employee.id,
                ...
                employee.displayId
            ].filter(Boolean).map(String); return keys.some(key => OrbisState.atRiskMap?.[key] || window.currentAtRiskRosterMap?.[key]);
        })
        .map(employee => `${employee.first || ''} ${employee.last || ''}`.trim())
        .filter(Boolean)
        .sort(compareText); const atRiskCount = Number(String(safeGet('kRisk')?.textContent || '0').trim()) || 0; setCardTitle(
            'cardAtRiskEmployees',
            ...
            .map(e => `${e.first || ''} ${e.last || ''}`.trim())
            .filter(Boolean)
            .sort(compareText); setCardTitle('cardImpactPlayers', impactPlayerNames, 'No impact players'); setCardTitle(
                'cardOnLeave',
    ...
    const disciplineCountText = String(safeGet('kOpenDiscipline')?.textContent || '').trim(); const hasRealDisciplineTooltip = existingDisciplineTooltip
        && existingDisciplineTooltip !== 'No open discipline cases'
        && existingDisciplineTooltip !== 'No discipline cases open'
        && existingDisciplineTooltip !== 'Could not load discipline cases'; if (!hasRealDisciplineTooltip) {
...
const reviewDate = safeGet('reviewDate')?.value || ''; const reviewType = safeGet('reviewType')?.value || ''; const reviewQuality = safeGet('reviewQuality')?.value || ''; const reviewAttendance = safeGet('reviewAttendance')?.value || ''; const reviewReliability = safeGet('reviewReliability')?.value || ''; const reviewCommunication = safeGet('reviewCommunication')?.value || ''; const reviewJudgement = safeGet('reviewJudgement')?.value || ''; const reviewInitiative = safeGet('reviewInitiative')?.value || ''; const reviewTeamwork = safeGet('reviewTeamwork')?.value || ''; const reviewKnowledge = safeGet('reviewKnowledge')?.value || ''; const reviewTraining = safeGet('reviewTraining')?.value || ''; const reviewOverallResult = safeGet('reviewOverallResult')?.value || ''; const reviewStrengths = safeGet('reviewStrengths')?.value || ''; const reviewImprovements = safeGet('reviewImprovements')?.value || ''; const reviewEmployeeComments = safeGet('reviewEmployeeComments')?.value || ''; const reviewManagerComments = safeGet('reviewManagerComments')?.value || '';    // Determine employee ID
const employeeId = currentEmployee?.dbId || currentEmployee?.id;
if (!employeeId) {
    showToast('No employee selected for review.', 'error');
    return;
}
const ratingToScore = (value) => { switch (String(value || '').trim()) { case 'Exceeds Expectations': return 4; case 'Meets Expectations': return 3; case 'Needs Improvement': return 2; case 'Unacceptable': return 1; default: return null; } }; const communicationScore = ratingToScore(reviewCommunication); const judgementScore = ratingToScore(reviewJudgement); const initiativeScore = ratingToScore(reviewInitiative); const knowledgeScore = ratingToScore(reviewKnowledge); const trainingScore = ratingToScore(reviewTraining); const blendedAttitudeInputs = [communicationScore, judgementScore, initiativeScore, knowledgeScore, trainingScore].filter(v => v !== null); const blendedAttitudeScore = blendedAttitudeInputs.length ? Math.round(blendedAttitudeInputs.reduce((sum, v) => sum + v, 0) / blendedAttitudeInputs.length) : null; const detailedCategorySummary = [`Quality of Work: ${reviewQuality || 'Not Rated'}`, `Attendance & Punctuality: ${reviewAttendance || 'Not Rated'}`, `Reliability / Dependability: ${reviewReliability || 'Not Rated'}`, `Communication Skills: ${reviewCommunication || 'Not Rated'}`, `Judgement & Decision-Making: ${reviewJudgement || 'Not Rated'}`, `Initiative & Flexibility: ${reviewInitiative || 'Not Rated'}`, `Cooperation & Teamwork: ${reviewTeamwork || 'Not Rated'}`, `Knowledge of Position: ${reviewKnowledge || 'Not Rated'}`, `Training & Development: ${reviewTraining || 'Not Rated'}`].join('\n');    // Prepare payload for Supabase
const payload = {
    employee_id: employeeId,
    review_date: reviewDate,
    review_type: reviewType,
    attendance_score: ratingToScore(reviewAttendance), performance_score: ratingToScore(reviewQuality), teamwork_score: ratingToScore(reviewTeamwork), attitude_score: blendedAttitudeScore, reliability_score: ratingToScore(reviewReliability),
...
if (typeof loadEmployeeReviews === 'function') {
    ...
    interface Window {
        EMPLOYEES?: any[];
        ALL_EMPLOYEES?: any[];
        currentAtRiskRosterMap?: Record<string, any>; currentImpactPlayerRosterMap?: Record<string, any>;[key: string]: any;
    }
} export { };